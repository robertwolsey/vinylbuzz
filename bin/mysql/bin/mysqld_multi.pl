...
$opt_help          = 0;
$opt_log           = undef();
$opt_mysqladmin    = "C:/mysql/pack/bin/mysqladmin";
$opt_mysqld        = "C:/mysql/pack/bin/mysqld";
$opt_no_log        = 0;
$opt_password      = undef();
$opt_tcp_ip        = 0;
$opt_user          = "root";
...
#### order:  my_print_defaults mysqld datadir, C:/mysql/pack/share
####
...
sub init_log
{
  ...
  if (!defined($logdir))
  {
    $logdir= "C:/mysql/pack/share" if (-d "C:/mysql/pack/share" && -w "C:/mysql/pack/share");
  }
  if (!defined($logdir))
  {
      ...
      if ("--datadir=" eq substr($options[$j], 0, 10)) {
        ...
        if (! -d $datadir."/mysql") {
          if (-w $datadir) {
            ...
            $install_cmd="C:/mysql/pack/bin/mysqld ";
...
#
#   Before using, you must create a my.cnf file either in C:/mysql/pack/my.cnf
...
[mysqld_multi]
mysqld     = C:/mysql/pack/bin/mysqld_safe
mysqladmin = C:/mysql/pack/bin/mysqladmin
user       = multi_admin
...
language   = C:/mysql/pack/share/mysql/english
user       = unix_user1
...
language   = C:/mysql/pack/share/mysql/swedish
user       = unix_user2
...
language   = C:/mysql/pack/share/mysql/estonia
user       = unix_user3
...
language   = C:/mysql/pack/share/mysql/japanese
user       = unix_user4
EOF
  exit(0);
}
